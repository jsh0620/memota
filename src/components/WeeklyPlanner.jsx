    ...
    <div className={`task-item ${task.done?'done':''} ${task.fromAI?'ai-made':''}`}>
      ...
      {editing ? (
        <input ref={inputRef} className="task-edit-input" value={draft}
          onChange={e => setDraft(e.target.value)} onBlur={commitEdit} onKeyDown={onKey}/>
      ) : (
...
export default function WeeklyPlanner() {
  ...
  const isWknd   = selectedDay === 'sat' || selectedDay === 'sun'
    ...
    <>
      ...
      <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
        ...
        {(weekTotal > 0 || todayTotal > 0) && (
          <div style={{ flexShrink:0, display:'flex', flexDirection:'column', gap:6, padding:'10px 14px', background:'var(--bg-3)', borderBottom:'1px solid var(--line)' }}>
            {weekTotal > 0 && (
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                <span style={{ fontSize:11, color:'var(--tx-3)', width:38, flexShrink:0 }}>이번 주</span>
                ...
                <span style={{ fontSize:11, fontWeight:700, color:'var(--gold)', width:32, textAlign:'right', flexShrink:0 }}>{weekPct}%</span>
              </div>
            )}
            {todayTotal > 0 && (
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                <span style={{ fontSize:11, color:'var(--tx-3)', width:38, flexShrink:0 }}>오늘</span>
                <div style={{ flex:1, height:3, background:'var(--line-2)', borderRadius:2, overflow:'hidden' }}>
                  <div style={{ height:'100%', width:`${todayPct}%`, background:'linear-gradient(90deg,var(--green),#6ee7b7)', borderRadius:2, transition:'width .5s' }}/>
                </div>
                <span style={{ fontSize:11, fontWeight:700, color:'var(--green)', width:32, textAlign:'right', flexShrink:0 }}>{todayPct}%</span>
              </div>
            )}
          </div>
        )}
        ...
        <div style={{ flex:1, minHeight:0, overflowY:'auto', WebkitOverflowScrolling:'touch', padding:'12px' }}>
          <div className={`day-card-mobile ${isToday(selDate)?'is-today':''} ${isWknd?'is-weekend':''}`}>
            ...
            <div className="task-area">
              {selTasks.length === 0 && (
                <div style={{ padding:'24px 10px', textAlign:'center', color:'var(--tx-3)', fontSize:12 }}>
                  할 일을 추가해보세요
                </div>
              )}
